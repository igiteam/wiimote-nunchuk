#!/bin/bash
# Wiimote - IR DEBUGGER WITH DYNAMIC MODE SWITCHING & QUARTZ MOUSE MOVEMENT (FIXED)
...
- (void)setReportingMode:(uint8_t)mode {
    ...
    printf("[Wiimote] Switched to Mode: 0x%02X\n", mode);
    fflush(stdout);
}
...
- (void)updateQuartzMousePosition {
    ...
    // FIXED: No horizontal inversion - moving right moves cursor right
    CGFloat normX = (CGFloat)self.irX1 / 1023.0;
...
- (void)parseExtendedIRData:(uint8_t *)irData {
    ...
    if (irData[0] != 0xFF || irData[1] != 0xFF) {
        ...
        self.irX1 = x;
        self.irY1 = self.irBottom ? (767 - y) : y;
    ...
    if (irData[3] != 0xFF || irData[4] != 0xFF) {
        ...
        self.irX2 = x;
        self.irY2 = self.irBottom ? (767 - y) : y;
    ...
    if (irData[6] != 0xFF || irData[7] != 0xFF) {
        ...
        self.irX3 = x;
        self.irY3 = self.irBottom ? (767 - y) : y;
    ...
    if (irData[9] != 0xFF || irData[10] != 0xFF) {
        ...
        self.irX4 = x;
        self.irY4 = self.irBottom ? (767 - y) : y;
...
- (void)parseBasicIRData:(uint8_t *)irData {
    if (!self.irEnabled) return;
    ...
    // Basic Mode Parsing (10 bytes for 4 objects)
    // Object 1
    if (irData[0] != 0xFF || irData[1] != 0xFF) {
        uint16_t x = irData[0] | ((irData[2] & 0x30) << 4);
        uint16_t y = irData[1] | ((irData[2] & 0xC0) << 2);
    ...
    // Object 2
    if (irData[3] != 0xFF || irData[4] != 0xFF) {
        uint16_t x = irData[3] | ((irData[2] & 0x03) << 8);
        uint16_t y = irData[4] | ((irData[2] & 0x0C) << 6);
        self.irX2 = x;
        self.irY2 = self.irBottom ? (767 - y) : y;
        self.irSize2 = 0;
    } else { self.irX2 = -1; self.irY2 = -1; }
    ...
    if (self.debugIR && (self.frameCount % 5 == 0)) {
        printf("[IR - Mode 0x%02X] Dots (Basic): ", self.currentMode);
        int dots = 0;
        if (self.irX1 != -1) { printf("P1:(%d,%d) ", self.irX1, self.irY1); dots++; }
        if (self.irX2 != -1) { printf("P2:(%d,%d) ", self.irX2, self.irY2); dots++; }
        if (self.irX3 != -1) { printf("P3:(%d,%d) ", self.irX3, self.irY3); dots++; }
        if (self.irX4 != -1) { printf("P4:(%d,%d) ", self.irX4, self.irY4); dots++; }
        if (dots == 0) printf("None");
        printf("\n");
        fflush(stdout);
    }
...
- (void)l2capChannelData:(IOBluetoothL2CAPChannel *)ch data:(void *)dp length:(size_t)len {
    ...
    // Parse IR depending on active mode
    if (reportID == 0x33 && len >= 17) {
        ...
        // Mode 0x37: 2-byte Buttons + 3-byte Accel + 6-byte Ext + IR
        // Try Extended Mode first (12 bytes at offset 11)
        uint8_t *irData = payload + 11;
        BOOL hasExtendedData = NO;
        for (int i = 0; i < 12; i++) {
            if (irData[i] != 0xFF) { hasExtendedData = YES; break; }
        }
        if (hasExtendedData) {
            [self parseExtendedIRData:irData];
        } else {
            // Fall back to Basic Mode (10 bytes at offset 11)
            [self parseBasicIRData:irData];
        }
...
echo -e "   ✅ Horizontal inversion fixed (moving right = cursor right)"
echo -e "   ✅ Mode 0x37 now supports both Extended (12-byte) and Basic (10-byte) IR"
echo -e "   ✅ Auto-detects IR format for mode 0x37"
echo -e "   ✅ Press + for Mode 0x33, - for Mode 0x37"
echo ""