...
- (void)onConnected {
    ...
    if (self.irEnabled) {
        ...
        // FIX 1: Use 0x06 instead of 0x04 (enable both pixel clock and logic)
...
- (void)initIR {
    ...
    // STEP 1: Write 0x01 to 0xB00030 (enable IR sensor)
    [self writeMemory:0xB00030 data:[NSData dataWithBytes:"\x01" length:1]];
    usleep(50000);
    
    // STEP 2: Write sensitivity blocks (Level 3)
    ...
    // FIX 2: Use Extended Mode (0x03) instead of Basic (0x01)
    uint8_t irMode = 0x03;  // Extended Mode - 12 bytes
    [self writeMemory:0xB00033 data:[NSData dataWithBytes:&irMode length:1]];
    usleep(50000);
    
    // STEP 4: Write 0x08 to 0xB00030 (enable IR with 8-bit mode)
    [self writeMemory:0xB00030 data:[NSData dataWithBytes:"\x08" length:1]];
    usleep(50000);
    
    // STEP 5: Initialize extension (CRITICAL for mode 0x37)
    [self initExtension];
    usleep(50000);
    
    // Set reporting mode to 0x37
...
// FIX 3: Correct Extended Mode IR parsing (12 bytes)
- (void)parseIRData:(uint8_t *)data {
    if (!self.irEnabled) return;
    
    // For mode 0x37 with Extended Mode: 12 bytes of IR data
    uint8_t *irData = data;
    ...
    // Parse Extended Mode (12 bytes for 4 objects)
    // Each dot is 3 bytes: X_Low, Y_Low, High (X_High, Y_High, Size)
    
    // Dot 1 (bytes 0, 1, 2)
    if (irData[0] != 0xFF || irData[1] != 0xFF || irData[2] != 0xFF) {
        uint16_t x1 = irData[0] | ((irData[2] & 0x30) << 4);
        uint16_t y1 = irData[1] | ((irData[2] & 0xC0) << 2);
        self.irX1 = x1;
        self.irY1 = self.irBottom ? (767 - y1) : y1;
        self.irSize1 = irData[2] & 0x0F;
    } else {
        self.irX1 = -1; self.irY1 = -1; self.irSize1 = 0;
    }
    
    // Dot 2 (bytes 3, 4, 5)
    if (irData[3] != 0xFF || irData[4] != 0xFF || irData[5] != 0xFF) {
        uint16_t x2 = irData[3] | ((irData[5] & 0x30) << 4);
        uint16_t y2 = irData[4] | ((irData[5] & 0xC0) << 2);
        self.irX2 = x2;
        self.irY2 = self.irBottom ? (767 - y2) : y2;
        self.irSize2 = irData[5] & 0x0F;
    } else {
        self.irX2 = -1; self.irY2 = -1; self.irSize2 = 0;
    }
    
    // Dot 3 (bytes 6, 7, 8)
    if (irData[6] != 0xFF || irData[7] != 0xFF || irData[8] != 0xFF) {
        uint16_t x3 = irData[6] | ((irData[8] & 0x30) << 4);
        uint16_t y3 = irData[7] | ((irData[8] & 0xC0) << 2);
        self.irX3 = x3;
        self.irY3 = self.irBottom ? (767 - y3) : y3;
    ...
    // Dot 4 (bytes 9, 10, 11)
    if (irData[9] != 0xFF || irData[10] != 0xFF || irData[11] != 0xFF) {
        uint16_t x4 = irData[9] | ((irData[11] & 0x30) << 4);
        uint16_t y4 = irData[10] | ((irData[11] & 0xC0) << 2);
        self.irX4 = x4;
        self.irY4 = self.irBottom ? (767 - y4) : y4;
...
- (void)parseNunchukData:(uint8_t *)decrypted withID:(uint8_t)id andCoreData:(uint8_t *)coreData {
    ...
    if (validDots > 0) {
        ...
        if (self.isScreenCalibrated) {
            ...
            screenY = (avgY / 767.0) * screenHeight;  // Extended Mode Y range is 0-767
        }
...
- (void)l2capChannelData:(IOBluetoothL2CAPChannel *)ch data:(void *)dp length:(size_t)len {
    ...
    if (id == 0x35 || id == 0x37) {
        ...
        // FIX 4: IR offset for mode 0x37 is 11 (Buttons 2 + Accel 3 + Extension 6)
        // Bytes: 0-1=Buttons, 2-4=Accel, 5-10=Extension, 11-22=IR (12 bytes)
        if (self.irEnabled && id == 0x37 && len >= 23) {
            [self parseIRData:d + 11];  // IR starts at offset 11
        }
...
echo -e "   ✅ Changed IR enable from 0x04 to 0x06"
echo -e "   ✅ Changed IR mode from Basic (0x01) to Extended (0x03)"
echo -e "   ✅ Fixed IR offset for mode 0x37 (now at offset 11)"
echo -e "   ✅ Fixed Extended Mode IR parsing (12 bytes, correct bit shifts)"
echo -e "   ✅ Changed Y range from 1023 to 767 for Extended Mode"
echo -e "   ✅ Added extension initialization"
echo ""
